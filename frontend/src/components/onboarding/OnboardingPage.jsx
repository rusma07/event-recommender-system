  ...
  const steps = [
    {
      ...
      subtitle: "Let's personalize your event discovery experience",
      icon: "🎉"
    },
    {
      title: "What are you interested in?",
      subtitle: "Select tags that match your interests",
      icon: "🎯"
    },
    {
      title: "You're all set!",
      subtitle: "Ready to discover amazing events",
      icon: "🎊"
    }
  ];

  const handleOnboardingComplete = (userData) => {
    console.log('Onboarding completed with:', userData);
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden">
        ...
        <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="text-center">
                <p className="text-gray-500 text-sm">
                  Selected: {userData.interests.length} tags
                </p>
          ...
          <div className="flex justify-between mt-12">
            ...
            <button
              onClick={nextStep}
              disabled={currentStep === 1 && userData.interests.length < 3}
              className={`px-8 py-3 rounded-lg font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg
                ${currentStep === 1 && userData.interests.length < 3
                  ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
                  : 'bg-blue-600 text-white hover:bg-blue-700'
                }`}
            >
              {currentStep === steps.length - 1 ? 'Start Exploring Events 🚀' : 'Continue →'}
            </button>
          </div>
        </div>
      </div>
